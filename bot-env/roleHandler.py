...
class roleHandler(commands.Cog):
    ...
    async def sortBySpreadsheet(self, ctx,
        ...
        embed.add_field(
            ...
            3. WILL ONLY WORK IF GROUP ROLES ARENT THE HIGHEST ROLES A MEMBER HAS. (fellowship role must be higher)
            """
        )
        embed.add_field(
            name = "NOTE:",
        ...
        if confirm:
            ...
            for row in recievedData:
                ...
                if location != "" and level != "" and m1 != "":
                    ...
                    for memberObj in [m1,m2,m3,m4]:
                        if not memberObj:
                            continue
                        await memberObj.add_roles(*[createdRole, locationRole, levelRole])
                        print("MADE IT HERE")

                    otherPeopleWhoHaveRole = [m for m in await peopleWhoHaveRole(ctx.guild, createdRole) if m not in [m1,m2,m3,m4]]
